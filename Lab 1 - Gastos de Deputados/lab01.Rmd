...
```{r}
colors = colorNumeric('Oranges', mapa@data$n_gastos)
...
```{r}
colors = colorNumeric('Reds', mapa@data$valor_gasto)
...
gastos_exterior <- ceap %>%
  ...
  group_by(sgUF) %>%
  summarise(gasto_ex = sum(valorLíquido))

mapa@data <- mapa@data %>%
  left_join(gastos_exterior)

colors = colorNumeric('Greens', mapa@data$gasto_ex)
...
mapa %>%
  ...
  addPolygons(opacity = 0.5,
              weight = 1,
              fillColor = colors(mapa@data$gasto_ex),
              fillOpacity = 1,
              color = "black",
              label = paste(mapa@data$NM_ESTADO),
              popup = paste(mapa@data$NM_ESTADO, "</br>",
                            "Total de gastos no exterior: ", ifelse(is.na(mapa@data$gasto_ex),
                                                                    0, mapa@data$gasto_ex))) %>%
  addLegend(position = "bottomright", pal = colors, values = mapa@data$gasto_ex,
            title = "Gastos no exterior",
            opacity = 1, na.label = "0")
```

O estado que contém os deputados que mais gastam no exterior é __Sâo Paulo__ seguido por _Minas Gerais_ o que já era de se esperas já que estes estão no topo do gasto geral de cota parlamentar também.

Por outro lado no __Mato Grosso do Sul, Goiás, Rio Grande do Norte e o Distrito Federal__ não foi observado nenhum gasto no exterior.  

#### Quais os deputados que mais ultrapassam o limite de CEAP do seu estado?

```{r}
limite_estado <- read.csv("limiteMensalCEAP.csv")

ceap_limite <- ceap %>%
  mutate(ano = substr(dataEmissao, 1, 4)) %>%
  mutate(mes = substr(dataEmissao, 6, 7)) %>%
  group_by(sgUF, ano, mes, nomeParlamentar) %>%
  summarise(total = sum(valorLíquido)) %>%
  left_join(limite_estado, by = c("sgUF" = "UF")) %>%
  mutate(dif = total - limite_mensal)


execedem_cota <- ceap_limite %>%
  filter(dif < 0) %>%
  group_by(nomeParlamentar) %>%
  summarise(n = n(),
            total_excedido = sum(total),
            media = mean(total),
            mediana = median(total))

execedem_cota %>%
  filter(total_excedido >= quantile(total_excedido, .99)) %>%
plot_ly(y =~reorder(nomeParlamentar, total_excedido), x= ~total_excedido, type="bar") %>%
  layout(title = "Parlamentares que mais excedem a cota",
         yaxis = list(title = ""),
         xaxis = list(title = "Total excedido"),
         margin = list(l = 130))
```

Observou-se que `r nrow(execedem_cota)` deputados execederam sua cota parlamentar em pelo menos uma vez no período entre 2014 e setembro de 2017.  

O gráfico acima mostra os 1% dos deputados que mais excederam a CEAP.
